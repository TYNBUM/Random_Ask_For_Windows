...
随问（RandomAsk）想解决一个很小、但每天都很烦的问题：明明只是想问 AI 一句话，却要先打开网页或 App，切窗口、等加载、找输入框、复制粘贴，再假装自己一点都不急。

RandomAsk 的思路很直接：把 AI 提问入口贴到桌面边上，需要时抬手就用。看到一段文字，拖一下；看到一块屏幕内容，框一下；剪贴板里刚好有东西，点一下。少一点仪式感，多一点快速响应。

它主打的不是把聊天界面做得更宏大，而是让“问一下 AI”这件事变轻：不用离开当前工作流，不用在 Web 和 App 之间来回穿梭，也不用为了一个小问题启动一整套聊天流程。
